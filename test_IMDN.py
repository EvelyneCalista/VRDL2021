...
parser.add_argument("--test_lr_folder", type=str, default='/media/bsplab/Disk/Evelyne_Class/Sem1/CV_DL/HW4/datasets/testing_lr_images/testing_lr_images',
                    help='the folder of the input images')
parser.add_argument("--output_folder", type=str, default='results')
parser.add_argument("--checkpoint", type=str, default='/media/bsplab/Disk/Evelyne_Class/Sem1/CV_DL/HW4/IMDN-master/checkpoint_prex2_x3/epoch_1000.pth',
...
for imname in filelist:
    ...
    # height, width, channels = im_l.shape
    # hx3 = height*3
    # wx3=width*3
    if len(im_l.shape) < 3:
    ...
    output_folder = os.path.join(opt.output_folder,
                                 imname.split('/')[-1].split('.')[0] +'_pred.png')

    if not os.path.exists(opt.output_folder):
        os.makedirs(opt.output_folder)
    out_img = out_img[:, :, [2, 1, 0]]
    # out_img = cv2.resize(out_img, (wx3,hx3), interpolation = cv2.INTER_CUBIC)
